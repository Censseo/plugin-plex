...
$user = $plex->getConfiguration('login');
$pass = $plex->getConfiguration('password');
$ip = $plex->getConfiguration('addr');
$port = $plex->getConfiguration('port');
if ($user == '' && $pass == '') {
    $link='http://'.$ip.':'.$port;
} else {
	$link='http://'.$user.':'.$pass.'@'.$ip.':'.$port;
}
